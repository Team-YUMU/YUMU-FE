...
import { myPagePwEditSchema } from '@/types/newPwdValidation';
...
export default function Edit({ setChangeUi }: EditProps) {
  ...
  const handleIntroduceEdit = () => {
    ...
    setValue('introduce', userData.introduce);
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className='flex  w-full flex-row items-center justify-center gap-8'>
      <div className='flex flex-col gap-5'>
        <span className='text-14-400'>프로필 정보 수정</span>
        <div className='flex flex-row gap-[8rem]'>
          {userData.profileImageUrl ? (
            <div className='border-1 h-[14.4rem] w-[14.4rem] rounded-[50rem] bg-gray-F'>
              <Image
                src={userData.profileImageUrl}
                width={144}
                height={144}
                alt='회원 이미지'
                className='border-1 h-full w-full rounded-[50rem] bg-gray-D'
              />
            </div>
          ) : (
            <Image
        ...
        {isEditing ? (
          <div className='relative'>
            <label className='text-14-400'>닉네임 변경</label>
            <AuthInput
              type='text'
        ...
        <div className='relative flex flex-col gap-[1rem]'>
          <p className='text-14-400'>작가 설명</p>
          {isIntroduceEditing ? (
            <>
            ...
            <>
              <p className='h-[8rem] w-[30.5rem]'>{userData.introduce}</p>
              <Button
                type='button'
                variant={'default'}
                size={'default'}
            ...
            <AlertDialogHeader>
              <AlertDialogDescription>
                ...
                <AlertDialogFooter className='flex flex-row items-center justify-center gap-2'>
                  ...
                  <AlertDialogAction className='w-full' type='submit' onClick={handleEditPwdClick}>
                    확인
                  </AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogDescription>
